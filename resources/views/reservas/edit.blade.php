...
<body class="bg-slate-950 text-slate-100 min-h-screen p-6 md:p-12 flex justify-center items-center font-sans">
    ...
    <div
        ...
        <div class="mb-6 pb-4 border-b border-slate-800">
            ...
            <p class="text-slate-400 text-sm mt-1">Modifica los detalles de la reserva seleccionada</p>
        </div>
        ...
        <form action="{{ route('reservas.update', $reserva->id) }}" method="POST" class="space-y-5">
                ...
                <select name="sala_id" id="sala_id" required
                    ...
                    @foreach ($salas as $sala)
                        <option value="{{ $sala->id }}"
                            {{ old('sala_id', $reserva->sala_id) == $sala->id ? 'selected' : '' }}>
            ...
            <div>
                <label for="cliente" class="block text-xs font-semibold text-slate-300 mb-2">Nombre del Cliente /
                    Empresa</label>
                <input type="text" name="cliente" id="cliente" value="{{ old('cliente', $reserva->cliente) }}"
                    required
                    class="w-full bg-slate-950 border border-slate-800 rounded-xl p-3.5 text-white text-sm focus:outline-none focus:border-amber-500 transition-colors">
                @error('cliente')
                    <span class="text-rose-400 text-xs mt-1 block">{{ $message }}</span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="datetime-local" name="fecha_inicio" id="fecha_inicio"
                        value="{{ old('fecha_inicio', date('Y-m-d\TH:i', strtotime($reserva->fecha_inicio))) }}"
                        required
                        class="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-white text-sm focus:outline-none focus:border-amber-500 transition-colors">
                    @error('fecha_inicio')
                        <span class="text-rose-400 text-xs mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="fecha_fin" id="fecha_fin"
                        value="{{ old('fecha_fin', date('Y-m-d\TH:i', strtotime($reserva->fecha_fin))) }}" required
                        class="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-white text-sm focus:outline-none focus:border-amber-500 transition-colors">
                    @error('fecha_fin')
                        <span class="text-rose-400 text-xs mt-1 block">{{ $message }}</span>
                    @enderror
            ...
            <div class="flex justify-end items-center gap-3 pt-4 border-t border-slate-800/80">
                ...
                <button type="submit"
                    class="bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-400 hover:to-orange-500 text-slate-950 font-bold py-2.5 px-6 rounded-xl shadow-lg shadow-amber-500/20 text-xs transition-all duration-300 transform active:scale-95">
                    Actualizar Reserva
                </button>
            </div>
        </form>
